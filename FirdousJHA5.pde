...
void draw() {
  exx = (0.5-noise(i/20+57))*5*cos(i/5)*(width+100)*noise(i/25+25) ;
  eyy = (0.5-noise(i/20))*5*sin(i/5)*(height+100)*noise(i/25 + 14);
  fill(0,20);
  ...
  for (float j = 0; j<=jay; j++) { //8
    ...
    for (int k = 0; k < kay+1; k++) {  
      ...
      //stroke(120-k*3);
      stroke(30);;
      undl(jay*kay*1.2-j*kay*1.2, 0,exx,eyy); //96 //k*(360/kay)+0*(0.6+j)+2.5*(j+3+noise(i/30))*sin((i+j)/2)+(10*noise(i/8)-2.5)
    }
  }
